...
const Intervals = () => {
    ...
    const [passedInterval, setPassedInterval] = useState({'name': '', 'correct': true, 'visible': false, 'triggered': false})
    ...
    <div className='w-screen h-screen'>
        ...
        {showMain && 
            <div className=' min-h-full pt-20'>
                ...
                <h1 className={`mt-16 opacity-0 transition-opacity duration-1000 ease-in-out text-5xl text-center font-montserrat ${passedInterval['correct'] ? 'text-green-500' : 'text-red-500'} ${passedInterval['visible'] ? 'opacity-100' : 'opacity-0'}`}>{passedInterval['name']}</h1>
            ...
            <div className='fixed top-0 left-0 w-full h-full flex items-center justify-center bg-zinc-800 bg-opacity-50 z-10 pt-20 pb-4'>
                <div className='bg-zinc-200 dark:bg-slate-700 max-h-full xsPlus:px-8 py-8 rounded-lg shadow-xl overflow-scroll hideScrollbar'>
                    <div className='flex flex-col items-center gap-4 dark:text-sky-200'>
                        ...
                        <div className='grid grid-cols-3 gap-x-16 gap-y-4 px-6 '>
                            {Object.entries(stats['intervalStats']).map(([interval, obj]) => 
                                <p key={interval}>{`${interval}: ${obj['correct']}/${obj['total']}`}</p>
                            )}
                        </div>
                        <div className='grid grid-cols-2'>
                            <button onClick={() => handleRestartClick(false)} className='bg-blue-300 border-2 border-blue-500 px-4 py-2 rounded-md mt-4 text-black font-light ml-4'>Restart session</button>
                            <button onClick={() => handleRestartClick(true)} className='bg-blue-300 border-2 border-blue-500 px-4 py-2 rounded-md mt-4 text-black font-light ml-4'>Restart with new settings</button>
                            <button onClick={() => handleExtendClick(false)} className='bg-blue-300 border-2 border-blue-500 px-4 py-2 rounded-md mt-4 text-black font-light ml-4'>Extend session</button>
                            <button onClick={() => handleExtendClick(true)} className='bg-blue-300 border-2 border-blue-500 px-4 py-2 rounded-md mt-4 text-black font-light ml-4'>Extend with new settings</button>
                        </div>
